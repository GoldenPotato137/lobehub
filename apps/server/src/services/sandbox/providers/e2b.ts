...
const log = debug('lobe-server:sandbox:e2b');
...
export class E2BSandboxProvider implements SandboxProvider {
  ...
  constructor(options: SandboxServiceOptions) {
    this.sessionManager = new E2BSandboxSessionManager();
  ...
  async exportFileToUploadUrl({
    ...
    try {
      ...
      try {
        ...
        return {
          error: { message: (retryError as Error).message, name: (retryError as Error).name },
          success: false,
        };
      }
    }
