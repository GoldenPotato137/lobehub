...
export class AiAgentService {
  ...
  async execAgent(params: InternalExecAgentParams): Promise<ExecAgentResult> {
    ...
    // as message relations via messagesFiles. Dedupe before passing — messages_files
    // has a composite PK on (file_id, message_id) and duplicates would abort the send.
    if (attachedFileIds && attachedFileIds.length > 0) {
      ...
      if (resolved.orderedFileIds.length > 0) {
        // Preserve caller-provided order on the message relation so chat
        // history renders attachments in the order the user uploaded them.
